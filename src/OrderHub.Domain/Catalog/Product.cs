...
public sealed class Product : IEstablishmentScopedEntity
{
    ...
    public void RemoveImage(Guid imageId) { if (!images.RemoveAll(image => image.Id == imageId).Equals(1)) throw new DomainException("Product image was not found."); }
    public void RemoveVariation(Guid variationId) { if (!variations.RemoveAll(variation => variation.Id == variationId).Equals(1)) throw new DomainException("Product variation was not found."); }
    ...
    private static string Normalize(string input, int max, string field) { var value = input.Trim(); if (value.Length is < 1 || value.Length > max) throw new DomainException($"{field} is invalid."); return value; }
}

public sealed class ProductAdditionalGroup
{
    private ProductAdditionalGroup() { }
    internal ProductAdditionalGroup(Guid tenantId, Guid establishmentId, Guid productId, Guid groupId, int order) { TenantId = tenantId; EstablishmentId = establishmentId; ProductId = productId; GroupId = groupId; Order = order; }
    public Guid TenantId { get; private set; }
    public Guid EstablishmentId { get; private set; }
