...
public enum PaymentStatus { Pending, Confirmed, Failed, Cancelled }
...
public sealed class PaymentMethod : IEstablishmentScopedEntity
{
    ...
    public void Update(string code, string name, bool isOnline, bool allowsChange, DateTimeOffset now) { Code = NormalizeCode(code); Name = Required(name, 100); IsOnline = isOnline; AllowsChange = allowsChange; UpdatedAt = now; }
    ...
    public static string NormalizeCode(string code) { var value = new string(code.Trim().Where(char.IsLetterOrDigit).ToArray()).ToUpperInvariant(); if (value.Length is < 2 or > 30) throw new DomainException("Payment-method code is invalid."); return value; }
    private static string Required(string value, int max) { var result = value.Trim(); if (result.Length is < 1 || result.Length > max) throw new DomainException("Payment-method name is invalid."); return result; }
}
...
public sealed class Payment : IEstablishmentScopedEntity
{
    ...
    public void Confirm(string? externalId, DateTimeOffset now) { EnsurePending(); ExternalId = NormalizeExternalId(externalId); Status = PaymentStatus.Confirmed; ConfirmedAt = now; Touch(now); }
    public void Fail(DateTimeOffset now) { EnsurePending(); Status = PaymentStatus.Failed; FailedAt = now; Touch(now); }
    public void Cancel(DateTimeOffset now) { if (Status is PaymentStatus.Cancelled or PaymentStatus.Failed) throw new DomainException("Payment cannot be cancelled from its current state."); Status = PaymentStatus.Cancelled; CancelledAt = now; Touch(now); }
    private void EnsurePending() { if (Status != PaymentStatus.Pending) throw new DomainException("Payment is not pending."); }
    private void Touch(DateTimeOffset now) { UpdatedAt = now; Version = Guid.NewGuid(); }
    private static string? NormalizeExternalId(string? value) { if (string.IsNullOrWhiteSpace(value)) return null; var result = value.Trim(); if (result.Length > 100) throw new DomainException("External payment id is invalid."); return result; }
}
