...
public enum CouponDiscountType { Percentage, FixedAmount }
...
public sealed class Coupon : IEstablishmentScopedEntity
{
    ...
    private static string? NormalizeDescription(string? value) { if (string.IsNullOrWhiteSpace(value)) return null; var normalized = value.Trim(); if (normalized.Length > 300) throw new DomainException("Coupon description is invalid."); return normalized; }
}
