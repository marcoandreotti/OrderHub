...
public sealed class PaymentMethodRepository(OrderHubDbContext context) : IPaymentMethodRepository
{
    ...
    public async Task AddAsync(PaymentMethod method, CancellationToken cancellationToken) { await context.PaymentMethods.AddAsync(method, cancellationToken); await SaveChangesAsync(cancellationToken); }
    public async Task SaveChangesAsync(CancellationToken cancellationToken) { try { await context.SaveChangesAsync(cancellationToken); } catch (DbUpdateException exception) when (exception.InnerException is PostgresException { SqlState: PostgresErrorCodes.UniqueViolation }) { throw new ConflictException("Payment-method code already exists."); } }
}
public sealed class PaymentRepository(OrderHubDbContext context) : IPaymentRepository
{
    ...
    public async Task AddAsync(Payment payment, CancellationToken cancellationToken) { await context.Payments.AddAsync(payment, cancellationToken); await SaveChangesAsync(cancellationToken); }
